...
@Slf4j
@Component
public class PublishPostScenario {
    ...
    public PublishPostScenario(LoadTestConfig config, DataSeeder dataSeeder) {
    ...
    public void execute() {
        ...
        String content = "Load test post #" + totalRequests.get() + " at " + System.currentTimeMillis();

        totalRequests.incrementAndGet();
        long start = System.currentTimeMillis();
        boolean success = apiClient.publishPost(token, content);
        ...
        if (totalRequests.get() % 50 == 0) {
            log.info("Publish progress: {} requests, {} success, {} failed, avg {}ms",
